...
def pearson_correlation(data_ext):
    ...
    np.savetxt(f'txt/{filename}correalation_matrix.txt',
               corr_matrix_txt,
               fmt='%s',
               delimiter='\t')
    ...
    plt.savefig(f'png/{filename}correlation_matrix_rel.png', bbox_inches='tight')
    plt.savefig(f'pdf/{filename}correlation_matrix_rel.pdf', bbox_inches='tight')
    plt.close()
    fig, ax = plt.subplots(dpi=DPI, figsize=FIGSIZE)
    im = ax.imshow(corr_matrix,
                   cmap=CMAP,
    ...
    cbar = ax.figure.colorbar(im, ax=ax, format='%.2f')
    cbar.set_label(label=CBARLABEL, size=FONTSIZE_LABELS)
    plt.savefig(f'png/{filename}correlation_matrix_abs.png', bbox_inches='tight')
    plt.savefig(f'pdf/{filename}correlation_matrix_abs.pdf', bbox_inches='tight')
    plt.close()
    # cbar = ax.figure.colorbar(im, ax=ax, format='%.2f')
    # cbar.set_label(label=CBARLABEL, size=FONTSIZE_LABELS)
    # plt.savefig(f'{png_path}/{filename}_correlation_matrix.png', bbox_inches='tight')
    # plt.savefig(f'{pdf_path}/{filename}_correlation_matrix.pdf', bbox_inches='tight')
    # plt.close()
    ...
    return corr_matrix, scanlist, filename
    ...
    for i in range(1, len(scans)):
        if scans[i] - scans[i-1] != 1:
            for j in range(1, scans[i] - scans[i-1]):
                missing_scans.append(str(i+j).zfill(zerofill))
    if len(missing_scans) > 0:
...
def echem_plotter(time, voltage, filename):
    print("Plotting electrochemistry...")
...
def pearson_echem_plotter(corr_matrix, scanlist, time, voltage, filename):
    ...
    axs[0].set_xlabel(AXESLABEL, fontsize=FONTSIZE_LABELS*1.2)
    axs[0].set_ylabel(AXESLABEL, fontsize=FONTSIZE_LABELS*1.2)
    axs[0].xaxis.set_label_position('top')
    axs[0].tick_params(axis='x', rotation=90)
    axs[0].tick_params(axis='both', labelsize=FONTSIZE_TICKS*2)
    # cbar = axs[0].figure.colorbar(im, ax=axs, format='%.2f')
    axs[1].plot(time, voltage, c=COLORS[1])
    axs[1].set_xlim(np.amin(time), np.amax(time))
    axs[1].set_ylim(VOLTAGEMIN, VOLTAGEMAX)
    axs[1].set_xlabel(XLABEL_ECHEM, fontsize=FONTSIZE_LABELS*1.2)
    axs[1].set_ylabel(YLABEL_ECHEM, fontsize=FONTSIZE_LABELS*1.2)
    axs[1].tick_params(axis='both', labelsize=FONTSIZE_TICKS*2)
    plt.subplots_adjust(hspace=0.1)
    cbar = plt.colorbar(im, ax=axs, anchor=(0,1), shrink=0.76)
    cbar.set_label(label=CBARLABEL, size=FONTSIZE_LABELS*1.2)
    plt.savefig(f'png/{filename}correlation_matrix_echem_abs.png', bbox_inches='tight')
    plt.savefig(f'pdf/{filename}correlation_matrix_echem_abs.pdf', bbox_inches='tight')
    plt.close()
    fig, axs = plt.subplots(dpi=DPI, figsize=(6,6), nrows=2, ncols=1,
                            gridspec_kw={'height_ratios': [1, 0.25],
                                         # 'width_ratios': [1, 0.1]
    ...
    cbar = plt.colorbar(im, ax=axs, anchor=(0,1), shrink=0.76)
    cbar.set_label(label=CBARLABEL, size=FONTSIZE_LABELS*1.2)
    plt.savefig(f'png/{filename}correlation_matrix_echem_rel.png', bbox_inches='tight')
    plt.savefig(f'pdf/{filename}correlation_matrix_echem_rel.pdf', bbox_inches='tight')
    plt.close()
...
def main():
    ...
    dummy_scan(data_ext)
    corr_matrix, scanlist, filename = pearson_correlation(data_ext)
    echemfile = list((Path.cwd() / "data").glob('*.txt'))[0]
    time, voltage = echem_collector(echemfile)
    echem_plotter(time, voltage, filename)
